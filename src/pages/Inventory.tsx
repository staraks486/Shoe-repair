...
export default function Inventory() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setShowAdd(false);
    setFormData({ name: '', category: 'Soles', quantity: 0, unit: 'pcs', minThreshold: 10 });
  };
  ...
  return (
    <div className="space-y-6">
      <header className="flex justify-between items-center bg-white p-6 border-b border-brand-border rounded-xl shadow-sm">
        <div>
          <h2 className="font-serif text-2xl font-bold text-brand-dark mb-1">Inventory Management</h2>
          <p className="text-xs font-sans text-brand-muted uppercase tracking-wider">Track and manage store supplies</p>
        </div>
        <button 
      ...
      {showAdd && (
        <form onSubmit={handleSubmit} className="bg-white p-6 border border-brand-border rounded-xl shadow-sm grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
